...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, batched_inputs, input_shape):
        ...
        if self.weak_aug:
            weak_losses = self.compute_losses(
                weak_items['outputs'],
                weak_items['targets'],
                weak_items['batched_inputs']
            )
        if self.strong_aug:
            strong_losses = self.compute_losses(
                strong_items['outputs'],
                strong_items['targets'],
                strong_items['batched_inputs'],
            )

        if self.weak_aug and (not self.strong_aug):
            return weak_losses
        elif (not self.weak_aug) and self.strong_aug:
            return strong_losses
        else:
            losses = dict()
            for k in weak_losses.keys():
                if ('prj' in k) and (not self.strong_with_proj_loss):
                    losses[k] = weak_losses[k]
                    continue
                elif ('pairwise' in k) and (not self.strong_with_pair_loss):
                    losses[k] = weak_losses[k]
                    continue
                else:
                    losses[k] = (weak_losses[k] + strong_losses[k]) / 2.
        if self.weight_dict['loss_consistency'] == 0:
            return losses

        # compute consistency loss
        weak_status = [{'overlap_bbox': input['overlap_bbox'],
                        'status'      : input['status'],
                        'img_size'    : input['image'].shape[-2:],
                        } for input in weak_items['batched_inputs']]
        strong_status = [{'overlap_bbox': input['overlap_bbox'],
                          'status'      : input['status'],
                          'img_size'    : input['image'].shape[-2:],
                          } for input in strong_items['batched_inputs']]
        losses['loss_consistency'] = self.loss_consistency(
            weak_items['outputs']['pred_masks'], weak_status,
            strong_items['outputs']['pred_masks'], strong_status, input_shape)
        if 'aux_outputs' in weak_items['outputs'] and 'aux_outputs' in strong_items['outputs']:
            for i, (w, s) in enumerate(zip(weak_items['outputs']['aux_outputs'],
                                           strong_items['outputs']['aux_outputs'])):
                losses[f'loss_consistency_{i}'] = self.loss_consistency(
                    w['pred_masks'], weak_status, s['pred_masks'], strong_status, input_shape)
        return losses
