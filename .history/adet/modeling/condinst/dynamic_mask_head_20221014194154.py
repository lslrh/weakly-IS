        ...
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    # compute consisce loss
                    loss_consistence = F.binary_cross_entropy_with_logits(mask_scores, (mask_scores_ema>0.5).float().detach(), reduction='mean')
                    show_feature_map((mask_scores>0.7).detach(), 2)
                    show_feature_map((mask_scores_ema>0.7).detach(), 3)
                    pdb.set_trace()
                    ...
                    if self._iter > 10000:
                        ...
                        # neg = (mask_scores < 0.5) 
                        # show_feature_map((mask_scores_mean>0.7).detach(), 2)
                        # show_feature_map((mask_scores_mean_2>0.7).detach(), 3)
                        # pdb.set_trace()
                    ...
                    losses.update({
                        ...
                        "loss_consistence": loss_consistence,
                    })
                else:
