...
class CondInst(nn.Module):
    ...
    def forward(self, batched_inputs):    
        ...
        if self.training:
            original_images_S  = [x["image_S"].to(self.device) for x in batched_inputs]
            original_images_ema = [x["image_ema"].to(self.device) for x in batched_inputs]
        else:
            original_images_S = original_images
        ...
        original_images_S = ImageList.from_tensors(original_images_S, self.backbone.size_divisibility)
        ...
        if "instances" in batched_inputs[0]:
            ...
            if self.training and self.copypaste and len(self.paste_data)>=self.memory_length and random_copypaste>=0.5:
                original_images_S, gt_instances = _copypaste_transform(original_images_S, gt_instances, self.paste_data)  
                original_images_S = ImageList.from_tensors(original_images_S, self.backbone.size_divisibility)
        
                # input_tensor =  original_images[0].clone()
                # input_tensor = input_tensor.to(torch.device('cpu')).permute(1,2,0).numpy()
                # input_tensor = cv2.cvtColor(input_tensor, cv2.COLOR_BGR2RGB)
                # input_tensor = Image.fromarray(input_tensor)
                # input_tensor.save('feat_visualize/img1_copypaste.png', 'png')

                # input_tensor =  original_images[1].clone()
                # input_tensor = input_tensor.to(torch.device('cpu')).permute(1,2,0).numpy()
                # input_tensor = cv2.cvtColor(input_tensor, cv2.COLOR_BGR2RGB)
                # input_tensor = Image.fromarray(input_tensor)
                # input_tensor.save('feat_visualize/img2_copypaste.png', 'png')

        # normalize images
        images_norm_S = [self.normalizer(x) for x in original_images_S]
        images_norm_S = ImageList.from_tensors(images_norm_S, self.backbone.size_divisibility)
        ...
        if "instances" in batched_inputs[0]:
            if self.boxinst_enabled:
                ...
                self.add_bitmasks(gt_instances, images_norm_S.tensor.size(-2), images_norm_S.tensor.size(-1))
        else:
            gt_instances = None 
  
        features = self.backbone(images_norm_S.tensor)
        mask_feats, sem_losses = self.mask_branch(features, gt_instances)

        proposals, proposal_losses = self.proposal_generator(
            images_norm_S, features, gt_instances, self.controller
        )
        ...
        if self.training:
            ...
            if self.ema_on:
                images_norm_ema = [self.normalizer(x) for x in original_images_ema]
                images_norm_ema = ImageList.from_tensors(images_norm_ema, self.backbone.size_divisibility)

                ema_features = self.backbone_ema(images_norm_ema.tensor)
                ...
                proposals_ema, proposal_losses_ema = self.proposal_generator_ema(
                    images_norm_ema, features, gt_instances, self.controller_ema
                )

                mask_losses = self._forward_mask_heads_train(proposals, proposals_ema, features, mask_feats, mask_feats_ema, gt_instances, batched_inputs)
                # paste mask
                paste_data={}
            ...
            padded_im_h, padded_im_w = images_norm_S .tensor.size()[-2:]
            processed_results = []
    ...
    def _forward_mask_heads_train(self, proposals, proposals_ema, features, mask_feats, mask_feats_ema, gt_instances, batched_inputs):
        ...
        loss_mask = self.mask_head(
            ...
            pred_instances, pred_instances_ema, gt_instances, batched_inputs
        )
        return loss_mask
