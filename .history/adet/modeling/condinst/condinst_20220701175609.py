...
class CondInst(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            if self.ema_on:
                ...
                mask_losses = self._forward_mask_heads_train(proposals, proposals_ema, mask_feats, ema_mask_feats, gt_instances)
                # update ema model
    ...
    def _forward_mask_heads_train(self, proposals, proposals_ema, mask_feats, ema_mask_feats, gt_instances):
        ...
        if self.max_proposals != -1:
            ...
            for im_id in range(num_images):
                ...
                instances_per_im_gt = pred_instances_ema[pred_instances_ema.im_inds == im_id]
                if len(instances_per_im) == 0:
                    kept_instances.append(instances_per_im)
                    kept_instances_ema.append(instances_per_im)
                    continue
