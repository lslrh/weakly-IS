...
def mask_focal_loss(x, targets, weights):
...
def mask_focal_loss_v2(x, targets, weights, alpha: float = 0.25, gamma: float = 2):
    ce_loss = F.binary_cross_entropy_with_logits(x, targets, weight=weights, reduction="none")
    return ce_loss.sum()/(weights.sum()*x.shape[1]+1)  
...
class DynamicMaskHead(nn.Module):
    ...
    def __call__(self, features, mask_feats, mask_feats_ema, mask_feat_stride, pred_instances, pred_instances_ema, gt_instances=None):
        if self.training:
            self._iter += 1
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    for i, x in enumerate(gt_instances):
                        # H, W, D = mask_feats_ema[i].shape
                        masks = torch.einsum('hwd,npd->nphw', mask_feats_ema[i], self.prototypes[x.gt_classes]).detach()
                        # feat_proto_distance = torch.norm(self.prototypes[x.gt_classes].reshape(-1,1,1,D).expand(-1,H,W,-1) - mask_feats_ema[i], 2, dim=-1).reshape(len(x.gt_classes),-1,H,W)
                        # masks = torch.exp(-feat_proto_distance/2)
                        proto_masks_list.append(masks/0.5)
                    ...
                    pdb.set_trace()
                    mask_scores_mean = scatter(mask_scores_ema*ious_weight.expand(-1,1,1,1), gt_inds.unsqueeze(1), dim=0, reduce="sum")[gt_inds]
                    pdb.set_trace()
                    ...
                    if self._iter > 0:
                        ...
                        pseudo_seg = proto_seg * mask_scores_mean
                        ...
                        thr = torch.clamp(thr, min=0.5, max=0.8)
                        pseudo_seg_final = ( pseudo_seg > thr) * gt_bitmasks.float()
                        
                        # show_feature_map_heat1(pseudo_seg_final.detach(), 2)
                        # show_feature_map_heat1((mask_scores_ema).detach(), 3)
                        # show_feature_map_heat1(((proto_seg>0.7)* gt_bitmasks.float()).detach(), 4)
                        # masks_2 = (mask_scores_mean > 0.55) * gt_bitmasks.float()
                        ...
                        loss_pseudo = (mask_focal_loss(mask_logits, pseudo_seg_final.detach(), weights)) * warmup_factor_2 * 0.3
                        ...
                        if 'paste_indicator' in gt_instances[0]._fields.keys():
                            ...
                            # pdb.set_trace()
                            loss_paste = mask_focal_loss(mask_logits, gt_masks, weights) * 1.0 + dice_coefficient(mask_scores, gt_masks, weights)
