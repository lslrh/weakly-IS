        ...
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    # input_tensor_ = cv2.cvtColor(input_tensor, cv2.COLOR_BGR2RGB)
                    # input_tensor = Image.fromarray(input_tensor_)
                    # input_tensor.save('feat_visualize/img1.png', 'png')
                    # (h, w) = batched_inputs[0]['image'].shape[1:]
                    # for i in range(len(gt_instances[0].gt_boxes)):   
                    #     box = gt_instances[0].gt_boxes[i].tensor[0]
                    #     h1,w1,h2,w2 = box[0].int(), box[1].int(), box[2].int(), box[3].int()

                    #     heat_map = np.zeros((h, w)) 
                    #     locations = pred_instances.locations[(pred_instances.im_inds==0) & (pred_instances.gt_inds==i)].cpu().numpy()
                    #     for j, l in enumerate(locations):
                    #         heat_map[int(l[1])][int(l[0])] = ious_weight[pred_instances.im_inds==0][j] 

                    #     img_= input_tensor_[w1:w2,h1:h2]
                        
                    #     heat_1 = heat_map/heat_map[w1:w2,h1:h2].max()
                    #     heat_1 = self.max_pooling_forward(heat_1, (16,16))*255  
                    #     heat_1 = cv2.resize(heat_1.astype(np.uint8), (w,h))
                    #     heat_1 = heat_1[w1:w2,h1:h2]

                    #     heat_img_ = cv2.applyColorMap(heat_1, cv2.COLORMAP_JET)
                    #     heat_img_ = cv2.cvtColor(np.asarray(heat_img_),cv2.COLOR_RGB2BGR)  
                        
                    #     add_img = cv2.addWeighted(img_, 0.5, heat_img_, 0.5, 0)
                    #     input_tensor = Image.fromarray(add_img)
                    #     input_tensor.save('feat_visualize/add/add_img'+str(i)+'.png')
                    ...
                    gt_bitmasks = F.interpolate(gt_bitmasks,
                                    size=mask_scores_ema.shape[-2:],
                                    mode="nearest")
                    ...
                    if self._iter > 0:
                        ...
                        pdb.set_trace()
                        gt_bitmasks = F.interpolate(mask_scores,
                                    size=mask_scores_ema.shape[-2:],
                                    mode="nearest")
