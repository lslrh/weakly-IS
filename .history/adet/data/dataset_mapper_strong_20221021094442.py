...
def build_ema_transform(cfg, is_train):
    """
    ...
    min_size = cfg.INPUT.MIN_SIZE_EMA
...
class DatasetMapperWithBasisPair(DatasetMapper):
    """
    ...
    def __init__(self, cfg, is_train=True):
        ...
        self.strong_gens = build_strong_transform_gen(cfg, is_train)
        self.augmentation_ema = build_ema_transform(cfg, is_train)
    ...
    def __call__(self, dataset_dict):
        ...
        image_ema, _ = T.apply_transform_gens(self.augmentation_ema, image) 
        dataset_dict["image_ema"] = torch.as_tensor(
            np.ascontiguousarray(image_ema.transpose(2, 0, 1))
        )

        # strong augmentation
        if random.random()>0.5:
            strong_gens = random.sample(self.strong_gens, 2)
            image_S, _ = T.apply_transform_gens(strong_gens, image)
        else:
            image_S = image
        dataset_dict["image_S"] = torch.as_tensor(
            np.ascontiguousarray(image_S.transpose(2, 0, 1))
        )
